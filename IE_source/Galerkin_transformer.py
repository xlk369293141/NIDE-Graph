...
class RelationalGraphAttention(nn.Module):
    ...
    def __init__(self, in_features,
        ...
        self.a = nn.Parameter(torch.FloatTensor(3*out_features, 1))
        xavier_normal_(self.a, gain=np.sqrt(2.0))

        self.leakyrelu = nn.LeakyReLU(self.alpha)

    def forward(self, node, adj, rel):
        h = torch.matmul(node, self.W)
        bsz, seq_len = h.size(0), h.size(1)

        re = torch.matmul(rel, self.W)
        r = re[adj] #(bsz, seq_len, seq_len, rel_dim)
        a_input = torch.cat([h.repeat(1, 1, seq_len).view(bsz, seq_len * seq_len, -1),
                             r.view(bsz, seq_len*seq_len, -1),
                             h.repeat(1, seq_len, 1)], dim=2)
        a_input = a_input.view(bsz, seq_len, -1, 3 * self.out_features)
        e = self.leakyrelu(torch.matmul(a_input, self.a).squeeze(-1))

        PAD_rel = rel.size(0) - 1 # PAD_id = 2*num_rel + 1
        zero_vec = -9e15*torch.ones_like(e)

        # attention = torch.where(adj !=PAD_rel, e, zero_vec)
        attention = torch.where(adj == PAD_rel, zero_vec, e)
        attention = F.softmax(attention, dim=-1)
        attention = F.dropout(attention, self.dropout, training=self.training)
        h_prime = torch.matmul(attention, h)
...
class spRelationalGraphAttention(nn.Module):
    """
    Simple spRGAT layer, modified from https://github.com/Diego999/pyGAT/blob/master/layers.py
    """
    ...
    def __init__(self, in_features,
                 ...
                 concat=True,
                 dropout=0.1):
        super(spRelationalGraphAttention, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.alpha = alpha
        self.concat = concat
    ...
    def forward(self, node, adj, rel):
        dv = 'cuda' if node.is_cuda else 'cpu'
        # adj: bsz x num_edge x 3 {[e_t, e_h, r]}
        # node: bsz x seq_len x feat_dim
        N = node.size(0)
        assert(adj.size(-1)==3)
        ...
        h = torch.matmul(node, self.W)
        # h: bsz x seq_len x out
        r = torch.mm(rel, self.W)
        # r: num_rel x out
        assert not torch.isnan(h).any()
        assert not torch.isnan(r).any()

        # Self-attention on the nodes - Shared attention mechanism
        edge_h = torch.cat((h[edge[0, :], :], r[edge[2,:]], h[edge[1, :], :]), dim=-1).t()
        # edge: 3*D x E
...
class RGAT(nn.Module):
    def __init__(self,
        ...
        '''
        if sparse:
            self.rgat_layer0 = spRelationalGraphAttention(in_features=node_feats,
                                            out_features=out_features,
                                            )
            self.rgat_layers = nn.ModuleList([copy.deepcopy(spRelationalGraphAttention(
                in_features=out_features,
                out_features=out_features,
            )) for _ in range(1, num_gcn_layers)])
        else:
            self.rgat_layer0 = RelationalGraphAttention(in_features=node_feats,
                                            out_features=out_features,
                                            )
            self.rgat_layers = nn.ModuleList([copy.deepcopy(RelationalGraphAttention(
                in_features=out_features,
                out_features=out_features,
            )) for _ in range(1, num_gcn_layers)])
        self.activation = activation
    ...
    def forward(self, x, edge, rel):
        '''
        input: node feats (-1, num_node, n_feats)
               edge only takes  (-1, num_node, num_node)
               if sparse, edge takes (-1, num_node, 3)
               edge matrix first one in the last dim is relational adj
               the second and later dim is graph Lap (option)
               In our implement, we just use relational adj
        '''
        # edge = edge[..., 0].contiguous()

        out = self.rgat_layer0(x, edge, rel)

        for layer in self.rgat_layers[:-1]:
            out = layer(out, edge, rel)
            if self.activation:
                out = self.relu(out)

        # last layer no activation
        return self.rgat_layers[-1](out, edge, rel)
...
class SpectralRegressor(nn.Module):
    def __init__(self, in_dim,
        ...
        self.spectral_conv = nn.ModuleList([spectral_conv(in_dim=n_hidden+spacial_dim,
                                                          out_dim=freq_dim,
                                                          n_grid=n_grid,
                                                          modes=modes,
                                                          dropout=dropout,
...
class SimpleTransformer(nn.Module):
    ...
    def forward(self, node, edge, pos, grid=None, weight=None, dynamical_mask=None):
        '''
        ...
        for encoder in self.encoder_layers:
            if self.return_attn_weight:
                x, attn_weight = encoder(x, pos, weight, dynamical_mask)
                attn_weights.append(attn_weight)
            else:
                x = encoder(x, pos, weight, dynamical_mask)
    ...
    def _get_regressor(self):
        if self.decoder_type == 'pointwise':
            self.regressor = PointwiseRegressor(in_dim=self.n_hidden+1,
            ...
            print(self.n_hidden)
            self.regressor = SpectralRegressor(in_dim=self.n_hidden+1,
                                               n_hidden=self.n_hidden+1,
...
class RelationalTransformer(nn.Module):
    ...
    def forward(self, x, pos=None, grid=None, weight=None, dynamical_mask=None):
        '''
        seq_len: num_node * time_size (include padding)
            the entity with first-order neighbors during input times
        node_feats: number of features of the inputs
        edge_feats: number of Laplacian matrices 
            we just use one dimension for relational adj
        pos_dim: dimension of the Euclidean space

        - node: (batch_size, seq_len, max_num_node, node_feats)
        - pos: (batch_size, seq_len, pos_dim)
        - edge: (batch_size, seq_len, max_num_node, max_num_node, edge_feats)
        - weight: (batch_size, seq_len, seq_len): mass matrix prefered
            or (batch_size, seq_len) when mass matrices are not provided
        ...
        # bsz, seq_len, num_node, node_feats = node.size()
        # x = self.feat_extract(node.view(-1,num_node,node_feats), edge.view(-1,num_node,num_node),rel)
        # x = x[:,0,:].view(bsz, seq_len, node_feats)
    ...
    def _get_feature(self):
        ...
        elif self.feat_extract_type == 'rgat' and self.num_feat_layers > 0:
            self.feat_extract = RGAT(node_feats=self.node_feats,
                                out_features=self.n_hidden,
                                num_gcn_layers=self.num_feat_layers,
                                activation=self.graph_activation,
                                sparse = self.sp_type,
    ...
    def _get_encoder(self):
        if self.attention_type in self.attention_types:
            encoder_layer = SimpleTransformerEncoderLayer(d_model=self.n_hidden+1,
                                                           n_head=self.n_head,
                                                           attention_type=self.attention_type,
                                                           dim_feedforward=self.dim_feedforward,
                                                           layer_norm=self.layer_norm,
            ...
            encoder_layer = _TransformerEncoderLayer(d_model=self.n_hidden+1,
                                                    nhead=self.n_head,
                                                    dim_feedforward=self.dim_feedforward,
                                                    layer_norm=self.layer_norm,
                                                    attn_weight=self.return_attn_weight,
...
class FourierTransformer2D(nn.Module):
    ...
    def _get_feature(self):
        if self.feat_extract_type == 'gcn' and self.num_feat_layers > 0:
            ...
            self.feat_extract = RGAT(node_feats=self.n_hidden,
                                out_features=self.n_hidden,
                                num_gcn_layers=self.num_feat_layers,
                                activation=self.graph_activation,                                
                                sparse = self.sp_type,
