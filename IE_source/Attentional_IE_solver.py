...
class Integral_attention_solver_multbatch:
    ...
    def __init__(
        ...
        if c is None:
            if self.initialization is True:
                c = lambda x: self.y_init
            else:
        ...
        if lower_bound is not None and upper_bound is not None:
            self.masking_map =  masking_function(lower_bound,upper_bound,n_batch=self.y_0.size(0))
            mask_times = torch.sort(torch.rand(self.samp))[0].to(device)*(self.x[-1]-self.x[0]) + self.x[0]
            self.mask = self.masking_map.create_mask(mask_times).to(device)
        else:
    ...
    def _solve_rhs_with_known_y(self, y: torch.Tensor) -> torch.Tensor:
        ...
        def integral(x):
            ...
            T = model.forward(y_in,dynamical_mask=None)
